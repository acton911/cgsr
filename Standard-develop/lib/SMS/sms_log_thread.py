...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'cmss_send_time', 'cmgw_error_times',
                          'cmss_error_times', 'cmss_receive_error_times', 'cmgd_error_times', 'cmgr_error_times', 'qftest_record',
    ...
    def write_result_log(self, runtimes):
        ...
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 8],
                                   2: ['cmss_send_time', 14], 3: ['cmgw_error_times', 16],
                                   4: ['cmss_error_times', 16], 5: ['cmss_receive_error_times', 24],
                                   6: ['cmgr_error_times', 24], 7: ['cmgd_error_times', 16],
                                   8: ['cefs_erasetimes', 10], 9: ['usrdata_erasetimes', 10]}
        ...
        cmss_send_time = '' if pd.isna(self.df.loc[runtimes, 'cmss_send_time']) else round(float(self.df.loc[runtimes, 'cmss_send_time']), 2)
        cmgw_error_times = int(self.df['cmgw_error_times'].sum())  # 写短信失败次数
        cmss_error_times = int(self.df['cmss_error_times'].sum())  # 发送短信失败次数
        cmss_receive_error_times = int(self.df['cmss_receive_error_times'].sum())  # 接收短信失败次数
        cmgr_error_times = int(self.df['cmgr_error_times'].sum())  # 读取短信失败次数
        cmgd_error_times = int(self.df['cmgd_error_times'].sum())  # 删除短信失败次数
        if runtimes == 1:
        ...
        result_list = [local_time, runtimes, cmss_send_time, cmgw_error_times, cmss_error_times, cmss_receive_error_times, cmgr_error_times, cmgd_error_times,
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        cmss_send_time_avg = np.round(np.mean(self.df['cmss_send_time'].dropna()), 2)  # 发送短信平均时间
        cmgw_error_times = int(self.df['cmgw_error_times'].sum())  # 写短信失败次数
        cmss_error_times = int(self.df['cmss_error_times'].sum())  # 发送短信失败次数
        cmss_receive_error_times = int(self.df['cmss_receive_error_times'].sum())  # 接收短信失败次数
        cmgr_error_times = int(self.df['cmgr_error_times'].sum())  # 读取短信失败次数
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                     ...
                     '发送短信时间(cmss_send_time_avg)：{}秒\n'.format(cmss_send_time_avg) + \
                     '写短信失败次数(cmgw_error_times)：{}次\n'.format(cmgw_error_times) + \
                     '发送短信失败次数(cmss_error_times): {}次\n'.format(cmss_error_times) + \
                     '接收短信失败次数(cmss_receive_error_times): {}次\n'.format(cmss_receive_error_times) + \
                     '读取短信失败次数(cmgr_error_times): {}次\n'.format(cmgr_error_times) + \
                     '删除短信失败次数(cmgd_error_times): {}次\n'.format(cmgd_error_times) + \
