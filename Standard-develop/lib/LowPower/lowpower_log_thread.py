...
class LowLogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'power_on_timestamp', 'driver_appear_timestamp',
                          ...
                          'sleep_real_rate', 'wake_curr_avg', 'wake_upset_times', 'wake_real_rate', 'qftest_record',
                          'cefs_erase_times',
                          'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        if self.vbat:  # 重启模块
            result_width_standard = {0: ['local_time', 25], 1: ['runtimes', 8], 2: ['check_usb_time', 14],
                                     3: ['power_on_time', 13], 4: ['power_on_fail_times', 19],
                                     5: ['net_fail_times', 16],
                                     ...
                                     12: ['cefs_erase_times', 10],
                                     13: ['usrdata_erase_times', 10]}
        else:
            result_width_standard = {0: ['local_time', 25], 1: ['runtimes', 8],
                                     ...
                                     6: ['wake_curr_value', 15], 7: ['wake_real_rate', 14], 8: ['cefs_erasetimes', 10],
                                     9: ['usrdata_erasetimes', 10]}
        ...
        if self.vbat:
            # 开关机参数统计
            power_on_timestamp = '' if self.df.loc[runtimes, 'power_on_timestamp'] == '' else self.df.loc[
                runtimes, 'power_on_timestamp']
            rn_timestamp = '' if self.df.loc[runtimes, 'rn_timestamp'] == '' else self.df.loc[
                runtimes, 'rn_timestamp']  # 接收到\r\n的时间戳，没有\r\n出现开机失败并且时间戳记为空('')
            driver_appear_timestamp = '' if self.df.loc[runtimes, 'driver_appear_timestamp'] == '' else self.df.loc[
                runtimes, 'driver_appear_timestamp']  # 驱动出现的时间戳，如果没有出现则为('')
            check_usb_time = 999 if power_on_timestamp is False or driver_appear_timestamp is False else round(
                (float(driver_appear_timestamp) - float(power_on_timestamp)), 2)
            power_on_time = 999 if power_on_timestamp is False or rn_timestamp is False or rn_timestamp is False else round(
                (float(rn_timestamp) - float(power_on_timestamp)), 2)
            ...
            # 结果统计
            result_string = ''
            result_list = [local_time, runtimes, check_usb_time, power_on_time, power_on_fail_times, net_fail_times,
                           sleep_upset_times,
            ...
            # 结果统计
            result_string = ''
            result_list = [local_time, runtimes, sleep_upset_times, sleep_curr_value, sleep_real_rate,
                           wake_upset_times,
                           wake_curr_value, wake_real_rate, cefs_erasetimes, usrdata_erasetimes]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        if self.vbat:
            usb_time_frame = self.df.loc[
                (self.df.power_on_timestamp.notnull()) & (self.df.driver_appear_timestamp.notnull()), [
                    'driver_appear_timestamp',
                    'power_on_timestamp']]  # 获取有driver_appear_timestamp和power_on_timestamp的runtimes
            check_usb_time_avg = round(
                (usb_time_frame['driver_appear_timestamp'] - usb_time_frame['power_on_timestamp']).sum() / len(
                    usb_time_frame['driver_appear_timestamp'] - usb_time_frame['power_on_timestamp']), 2)  # 计算平均时间
            power_on_time_frame = self.df.loc[
                (self.df.power_on_timestamp.notnull()) & (self.df.rn_timestamp.notnull()), ['rn_timestamp',
                                                                                            'power_on_timestamp']]  # 获取有rn_timestamp和power_on_timestamp的runtimes
            power_on_time_avg = round(
                (power_on_time_frame['rn_timestamp'] - power_on_time_frame['power_on_timestamp']).sum() / len(
                    power_on_time_frame['rn_timestamp'] - power_on_time_frame['power_on_timestamp']), 2)  # 计算平均时间
            power_on_fail_times = list(self.df['rn_timestamp']).count('')
            # 新增统计flash擦除次数
