...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'ATD_Fail_times', 'qftest_record', 'cefs_erase_times', 'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['ATD_Fail_times', 20], 3: ['cefs_erasetimes', 10], 4: ['usrdata_erasetimes',10]}
        ...
        atd_fail_times = int(self.df['ATD_Fail_times'].sum())
        ...
        result_list = [local_time, runtimes, atd_fail_times, cefs_erasetimes, usrdata_erasetimes]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        atd_fail_times = int(self.df['ATD_Fail_times'].sum())  # CFUN切换失败次数
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 '拨打电话失败次数(ATD_Fail_times)：{}次\n'.format(atd_fail_times) + \
