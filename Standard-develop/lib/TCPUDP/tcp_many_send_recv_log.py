...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'many_send_success_times', 'many_send_fail_times',
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['many_send_success_times', 20],
                                   3: ['many_send_fail_times', 20], 4: ['cefs_erase_times', 10],
                                   5: ['usrdata_erase_times', 10]}
        ...
        many_send_success_times = int(self.df['many_send_success_times'].sum())
        many_send_fail_times = int(self.df['many_send_fail_times'].sum())
        if runtimes == 1:
        ...
        result_list = [local_time, runtimes, many_send_success_times, many_send_fail_times, cefs_erasetimes,
                       usrdata_erasetimes]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        many_send_success_times = int(self.df['many_send_success_times'].sum())   # 发送成功次数
        many_send_fail_times = int(self.df['many_send_fail_times'].sum())  # 发送失败次数
        # 统计flash参数平均值，最大值
        cefs_erasetotal = 0
        ufs_erasetotal = 0
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 '多路连续收发发送成功次数(many_send_success_times)：{}次\n'.format(many_send_success_times) + \
                 '多路连续收发发送失败次数(many_send_fail_times)：{}次\n'.format(many_send_fail_times) + \
