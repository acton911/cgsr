...
class LogThread(Thread):
    def __init__(self, version, info, log_queue, connection_mode, cpin_flag, main_queue):
        super().__init__()
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'buffer_send_start_timestamp', 'buffer_send_end_timestamp',
                          'buffer_send_fail_times', 'buffer_send_success_times', 'push_send_start_timestamp',
                          'push_send_end_timestamp', 'push_send_fail_times', 'push_send_success_times', 'cefs_erase_times',
                          'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['buffer_send_time', 20],
                                   3: ['buffer_send_success_times', 20], 4: ['buffer_send_fail_times', 20],
                                   5: ['push_send_time', 20], 6: ['push_send_success_times', 20],
                                   7: ['push_send_fail_times', 20], 8: ['cefs_erase_times', 10],
        ...
        buffer_send_time = '' if pd.isna(self.df.loc[runtimes, 'buffer_send_start_timestamp']) or pd.isna(self.df.loc[runtimes, 'buffer_send_end_timestamp']) else np.round(self.df.loc[runtimes, 'buffer_send_end_timestamp'] - self.df.loc[runtimes, 'buffer_send_start_timestamp'], 2)
        buffer_send_success_times = int(self.df['buffer_send_success_times'].sum())
        buffer_send_fail_times = int(self.df['buffer_send_fail_times'].sum())
        push_send_time = '' if pd.isna(self.df.loc[runtimes, 'push_send_start_timestamp']) or pd.isna(self.df.loc[runtimes, 'push_send_end_timestamp']) else np.round(self.df.loc[runtimes, 'push_send_end_timestamp'] - self.df.loc[runtimes, 'push_send_start_timestamp'], 2)
        push_send_success_times = int(self.df['push_send_success_times'].sum())
        push_send_fail_times = int(self.df['push_send_fail_times'].sum())
        if runtimes == 1:
        ...
        result_list = [local_time, runtimes, buffer_send_time, buffer_send_success_times, buffer_send_fail_times,
                       push_send_time, push_send_success_times, push_send_fail_times, cefs_erasetimes,
                       usrdata_erasetimes]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        buffer_send_success_times = int(self.df['buffer_send_success_times'].sum())   # 发送成功次数
        buffer_send_fail_times = int(self.df['buffer_send_fail_times'].sum())  # 发送失败次数
        buffer_send_a_b_frame = self.df.loc[(self.df.buffer_send_start_timestamp.notnull()) & (self.df.buffer_send_end_timestamp.notnull()), ['buffer_send_start_timestamp', 'buffer_send_end_timestamp']]
        buffer_send_time_avg = np.round(np.mean(buffer_send_a_b_frame['buffer_send_end_timestamp'] - buffer_send_a_b_frame['buffer_send_start_timestamp']), 2)
        push_send_success_times = int(self.df['push_send_success_times'].sum())  # 发送成功次数
        push_send_fail_times = int(self.df['push_send_fail_times'].sum())  # 发送失败次数
        push_send_a_b_frame = self.df.loc[(self.df.push_send_start_timestamp.notnull()) & (self.df.push_send_end_timestamp.notnull()), ['push_send_start_timestamp', 'push_send_end_timestamp']]
        push_send_time_avg = np.round(np.mean(push_send_a_b_frame['push_send_end_timestamp'] - push_send_a_b_frame['push_send_start_timestamp']), 2)
        # 统计flash参数平均值，最大值
        cefs_erasetotal = 0
        ufs_erasetotal = 0
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 'buffer模式发送成功次数(buffer_send_success_times)：{}次\n'.format(buffer_send_success_times) + \
                 'buffer模式发送失败次数(buffer_send_fail_times)：{}次\n'.format(buffer_send_fail_times) + \
                 'buffer模式发送时间平均时间(buffer_send_time_avg)：{}秒\n'.format(buffer_send_time_avg) + \
                 'push模式发送成功次数(push_send_success_times): {}次\n'.format(push_send_success_times) + \
                 'push模式发送失败次数(push_send_fail_times)：{}次\n'.format(push_send_fail_times) + \
                 'push模式发送发送时间平均时间(push_send_time_avg)：{}次\n'.format(push_send_time_avg) + \
