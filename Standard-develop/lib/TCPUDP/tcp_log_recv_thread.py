...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'send_fail_times', 'send_success_times',
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['send_success_times', 20],
                                   3: ['send_fail_times', 20], 4: ['cefs_erase_times', 10], 5: ['usrdata_erase_times', 10]}
        ...
        send_success_times = int(self.df['send_success_times'].sum())
        send_fail_times = int(self.df['send_fail_times'].sum())
        ...
        result_list = [local_time, runtimes, send_success_times, send_fail_times, cefs_erasetimes, usrdata_erasetimes]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        send_success_times = int(self.df['send_success_times'].sum())   # 发送成功次数
        send_fail_times = int(self.df['send_fail_times'].sum())  # 发送失败次数
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 '模块接收数据成功次数(recv_success_times)：{}次\n'.format(send_success_times) + \
                 '模块接收数据失败次数(recv_fail_times)：{}次\n'.format(send_fail_times) + \
