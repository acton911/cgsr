...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'tran_send_start_timestamp', 'tran_send_end_timestamp',
                          'tran_send_fail_times', 'tran_send_success_times', 'qftest_record', 'cefs_erase_times',
                          'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['tran_send_time', 20],
                                   3: ['tran_send_success_times', 20], 4: ['tran_send_fail_times', 20],
                                   5: ['cefs_erasetimes', 10], 6: ['usrdata_erasetimes', 10]}
        ...
        tran_send_time = '' if pd.isna(self.df.loc[runtimes, 'tran_send_start_timestamp']) or pd.isna(self.df.loc[runtimes, 'tran_send_end_timestamp']) else np.round(self.df.loc[runtimes, 'tran_send_end_timestamp'] - self.df.loc[runtimes, 'tran_send_start_timestamp'], 2)
        buffer_send_success_times = int(self.df['tran_send_success_times'].sum())
        buffer_send_fail_times = int(self.df['tran_send_fail_times'].sum())
        ...
        result_list = [local_time, runtimes, tran_send_time, buffer_send_success_times, buffer_send_fail_times, cefs_erasetimes,
                       usrdata_erasetimes]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        tran_send_success_times = int(self.df['tran_send_success_times'].sum())   # 发送成功次数
        tran_send_fail_times = int(self.df['tran_send_fail_times'].sum())  # 发送失败次数
        tran_send_a_b_frame = self.df.loc[(self.df.tran_send_start_timestamp.notnull()) & (self.df.tran_send_end_timestamp.notnull()), ['tran_send_end_timestamp', 'tran_send_start_timestamp']]
        tran_send_time_avg = np.round(np.mean(tran_send_a_b_frame['tran_send_end_timestamp'] - tran_send_a_b_frame['tran_send_start_timestamp']), 2)
        # 统计flash参数平均值，最大值
        cefs_erasetotal = 0
        ufs_erasetotal = 0
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 '透传模式发送成功次数(tran_send_success_times)：{}次\n'.format(tran_send_success_times) + \
                 '透传模式发送失败次数(tran_send_fail_times)：{}次\n'.format(tran_send_fail_times) + \
                 '透传模式发送时间平均时间(tran_send_time_avg)：{}秒\n'.format(tran_send_time_avg) + \
