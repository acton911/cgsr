...
class DSSSManager:
    def __init__(self, main_queue, route_queue, uart_queue, at_queue, log_queue, at_port, uart_port, version, imei,
    ...
    def init(self):
        ...
        if main_queue_data is False:
            return False
        # 6. dsss双卡切换指令初始化
        main_queue_data = self.route_queue_put('AT', 'dsss_init', self.runtimes)
        if main_queue_data is False:
            return False
        # 7. 关闭AT口
