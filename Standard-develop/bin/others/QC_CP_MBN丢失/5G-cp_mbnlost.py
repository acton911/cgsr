...
压测逻辑
SIM1:00101、SIM2：实网卡
1、SIM1切换至SIM2，AT+QUIMSLOT=2
2、选择MBN，at+qmbncfg="select","CDMAless-Verizon" 该指令无需关注是否有返回直接上报超时
3、模组会自动重启，重启后，检查MBN列表AT+QMBNCFG="list"
4、SIM2切换至SIM1，AT+QUIMSLOT=1
5、选择MBN，at+qmbncfg="select","ROW_Generic_3GPP_PTCRB_GCF" 该指令无需关注是否有返回直接上报超时
6、模组会自动重启，重启后，检查MBN列表AT+QMBNCFG="list"
7、重复步骤1-6
...
at_port = 'COM38'  # 定义AT端口号,用于发送AT指令或检测URC信息
dm_port = 'COM40'  # 定义DM端口号,用于判断模块有无dump
debug_port = ''  # RM模块参数设置为''，但是需要连接跳线读取debug log，RG需要设置为DEBUG UART串口号
revision = 'RM520NGLAAR01A06M4G'  # 设置Revision版本号，填写ATI+CSUB查询的Revision部分
...
runtimes_temp = 6  # 设置脚本运行次数，默认0是不限次数，如果设置1000则脚本运行1000个runtimes后自动结束
...
log_thread = LogThread(version, dss_info, log_queue, '', False, main_queue)
...
while True:
    ...
    # 卡槽切换 从1->2 或者2->1
    main_queue_data = route_queue_put('AT', 'switch_slot', runtimes)
    if main_queue_data is False:
        dsss_manager.dsss_init()
        continue

    # 检测驱动消失
    main_queue_data = route_queue_put('Process', 'check_usb_driver_dis', False, runtimes)
    if main_queue_data is False:
        dsss_manager.dsss_init()  # 开机失败初始化
        continue
    ...
    # 检查mbn列表
    time.sleep(10)
    main_queue_data = route_queue_put('AT', 'check_mbn_lost', runtimes)
    if main_queue_data is False:
        dsss_manager.dsss_init()  # 开机失败初始化
        continue
