...
3. 手动配置开启rgmii或rtl8125模式
    【RGMII】AT+QETH="rgmii","enable",1
    【PCIE】at+qcfg="pcie/mode",1   at+qeth="eth_driver","r8125"/"r8168"  [该指令重启生效]
4. Python版本大于3.8
5. 需打开adb功能
脚本逻辑:
1.脚本初始化检测版本信息,IMEI,CPIN
2.打开AT口
3.检测注网
4.flash分区擦写量查询
5.配置桥接ippt模式
6.检测获取到外网IP
7.外网ip下进行ping百度
8.删除ippt规则(参数is_waitrule控制是否等待该删除规则指令返回OK)
9.关闭AT口
10.删除规则指令返回OK或不返回OK后0~11秒内进行vbat断电重启
11.重复2-10
"""
...
dial_mode = 'RTL8125'  # 设置拨号方式 RGMII或者RTL8125 用于log名称
ping_times = 60  # 默认每个runtimes ping 100次
is_waitrule = True  # 是否等待删除规则指令返回 True 等待  False 不等待
...
# ======================================================================================================================
# 其他参数
restart_mode = 1  # vbat开关机 该压力为开关机压力不可更改
runtimes = 0
version = revision + sub_edition
script_start_time = time.time()  # 脚本开始的时间，用作最后log统计
dial_info = '{}_开关机断开拨号{}规则返回获取公网IP'.format(dial_mode, '等待' if is_waitrule else '不等待')  # 脚本详情字符串
...
while True:
    ...
    if is_waitrule:  # 等待删除规则指令返回OK
        # 配置IPPT模式
        main_queue_data = route_queue_put('AT', 'set_ipptmode', runtimes)
        if main_queue_data is False:
            Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
            continue
        ...
        time.sleep(10)
        # 桥模式下检查是否获取到外网ip
        main_queue_data = route_queue_put('Process', 'check_outnet_ip', runtimes)
        if main_queue_data is False:
            Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
            continue
        ...
        time.sleep(2)
        # 配置 at+QMAP="mPDN_rule",0
        main_queue_data = route_queue_put('AT', 'set_rulewait', runtimes)
        if main_queue_data is False:
            Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
            continue

        if restart_mode == 1:
            main_queue_data = Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
            if main_queue_data is False:
                continue

    else:  # 不等的删除规则指令返回直接断电
        # 配置IPPT模式
        main_queue_data = route_queue_put('AT', 'set_ipptmode', runtimes)
        if main_queue_data is False:
            Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
        ...
        time.sleep(30)
        # 进行PING连接
        main_queue_data = route_queue_put('Process', 'ping', ping_url, ping_times, ping_4_6, ping_size,
                                          runtimes)
        if main_queue_data is False:
            Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
            continue
        ...
        time.sleep(2)
        # 配置 at+QMAP="mPDN_rule",0
        main_queue_data = route_queue_put('AT', 'set_rulenowait', runtimes)
        if main_queue_data is False:
            Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
            continue
