...
restart_mode = 1  # 设置是否开关机，None：每个runtimes不进行开关机，1：每个runtimes进行VBAT开关机
...
restart_mode_str = "VBAT" if restart_mode == 1 else '正常'
dial_info = '{}_{}_PING连接稳定性'.format(restart_mode_str, dial_mode)  # 脚本详情字符串
...
while True:
    ...
    if restart_mode is not None:
        dial_init_fusion_info = Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
