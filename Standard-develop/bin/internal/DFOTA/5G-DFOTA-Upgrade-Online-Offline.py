...
at_port = 'COM16'  # 定义AT端口号,用于发送AT指令或检测URC信息
...
sub_edition = 'V04'  # 设置SubEdition版本号，填写ATI+CSUB查询的SubEdition部分
after_upgrade_revision = "RG520FNAEAR03A01M4G"  # 设置升级后的Revision版本号，填写ATI+CSUB查询的Revision部分
after_upgrade_sub_edition = "V03"  # 设置升级后的SubEdition版本号，填写ATI+CSUB查询的SubEdition部分
a_b_package_path = r'FTP://test:test@ftp2.quectel.com/Acton/V04-V03.zip'  # a到b版本的升级包路径，精确到包名，路径可以是本地电脑路径，也可是FTP/HTTP/HTTPS/
b_a_package_path = r'FTP://test:test@ftp2.quectel.com/Acton/V03-V04.zip'  # b到a版本的升级包路径，精确到包名，路径可以是本地电脑路径，也可是FTP/HTTP/HTTPS/
...
multi_vbat = True  # Multifota 升级过程是否断电 True：升级过程中断电  False: 升级过程中不断电
...
while True:
    ...
    for mode in ('forward', 'backward'):  # forward代表正向升级，backward代表逆向升级
        ...
        for i in range(1, 5):
            ...
            # 检测DFOTA升级
            main_queue_data = route_queue_put('AT', 'check_dfota_status', [version, after_upgrade_version], package_error_list, upgrade_error_list, mode, multi_vbat, VBAT, is_x6x, runtimes)
            if main_queue_data is False:
                dfota_manager.dfota_upgrade_exception_init()
                continue
        ...
        while True:
            ...
            if multi_vbat:
                dfota_status = dfota_manager.multi_dfota_vbat()
                if dfota_status is False:
                    continue
            # 检测MultiFota 升级开始标志
            # main_queue_data = route_queue_put('uart', 'check_MultiFota_start', 5, runtimes)
            # if main_queue_data is False:
            #     dfota_manager.dfota_upgrade_exception_init()
            #     continue
            #
            ...
            # 检测MultiFota 升级完成标志
            # if is_x6x is True:
            #     main_queue_data = route_queue_put('uart', 'check_MultiFota_end', 5, runtimes)
            #     if main_queue_data is False:
            #         dfota_manager.dfota_upgrade_exception_init()
            #         continue
            # else:
            #     main_queue_data = route_queue_put('uart', 'check_MultiFota_ending', 5, runtimes)
            #     if main_queue_data is False:
            #         dfota_manager.dfota_upgrade_exception_init()
            #         continue
