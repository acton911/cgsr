...
from Linux_apn_and_auth import linuxAPNandAuthFunctions
import sys
import pickle
...
# debug
all_logger.info("LinuxQSSFunctions: {} , type{}".format(LinuxQSSFunctions, type(LinuxQSSFunctions)))
...
cat_dl = args_5g.dev_cat_dl
cat_ul = args_5g.dev_cat_ul
rgmii_ethernet_name = args_5g.dev_rgmii_ethernet_name
rtl8125_ethernet_name = args_5g.dev_rtl8125_ethernet_name   # RTL8125网卡名
...
params_dict = {
    ...
    "cat_dl": cat_dl,
    "cat_ul": cat_ul,
    'rgmii_ethernet_name': rgmii_ethernet_name,
    'rtl8125_ethernet_name': rtl8125_ethernet_name,
    'eth_test_mode': eth_test_mode,
    'pc_ethernet_name': pc_ethernet_name,
}

all_logger.info('进行测试项:{}: {}'.format(func, case_setting['summary']))
exec("LinuxQSSFunctions(**{}).{}()".format(params_dict, func))
