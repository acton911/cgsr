...
class IPerfServer(Thread):
    ...
    def __init__(self, ip, user, passwd, ssh_port):
        super().__init__()
        self.ip = ip
        self.user = user
        self.passwd = passwd
...
def iperf(ip="112.31.84.164", user='Q', passwd='st', port=19999, mode=3, times=100, bandwidth='10M', interval=None, bind=None, parallel=None, length=None, window=None, mtu=None, omit=None):
    """
    iPerf3.9 wrapper.
    ...
    print('[iperf3 cmd] {}'.format(' '.join(iperf_cmd)))
