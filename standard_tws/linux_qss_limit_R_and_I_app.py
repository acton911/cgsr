...
from linux_qss_limit_R_and_I import LinuxQSSLimitRAndI
import sys
import pickle
...
# debug
all_logger.info("LinuxQSSLimitRAndI: {} , type{}".format(LinuxQSSLimitRAndI, type(LinuxQSSLimitRAndI)))
...
no_limit_firmware_path = args_5g.no_limit_firmware_path
no_limit_name_sub_version = args_5g.no_limit_name_sub_version
oc_num = args_5g.dev_oc_num


# 解压的属性
at_port, dm_port, uart_port, nema_port, debug_port = main_device['usbat'], main_device['upgrade'], main_device['uart'], main_device['usbNmea'], main_device['debug']
...
params_dict = {
    ...
    'prev_firmware_path': no_limit_firmware_path,
    'prev_name_sub_version': no_limit_name_sub_version,
    'oc_num': oc_num,
}

all_logger.info('进行测试项:{}: {}'.format(func, case_setting['summary']))
exec("LinuxQSSLimitRAndI(**{}).{}()".format(params_dict, func))
