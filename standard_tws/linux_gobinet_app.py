...
from linux_gobinet import LinuxGobiNet
import sys
import pickle
...
# debug
all_logger.info("LinuxGobiNet: {} , type{}".format(LinuxGobiNet, type(LinuxGobiNet)))
...
gobinet_network_card_name = args_5g.dev_gobinet_network_card_name
gobinet_driver_path = args_5g.dev_gobinet_driver_path
...
params_dict = {
    ...
    "gobinet_network_card_name": gobinet_network_card_name,
    "gobinet_driver_path": gobinet_driver_path,
    "extra_ethernet_name": pc_ethernet_name,
    "params_path": params_path
}

all_logger.info('进行测试项:{}: {}'.format(func, case_setting['summary']))
exec("LinuxGobiNet(**{}).{}()".format(params_dict, func))
