...
class UpgradeOnOff:
    ...
    def compare_file(self):
        ...
        different_list1 = ['efuse']   # X6X RM项目工厂包多了个efuse文件夹
        ...
        for path, firmware_path in zip(['prev', 'cur'], [self.prev_firmware_path, self.firmware_path]):
            ...
            for file in os.listdir(current_path):
                if not filecmp.cmp(os.path.join(current_path, file), os.path.join(firmware_path, file)):
                    raise UpgradeOnOffError("复制文件对比异常")
    ...
    def qfil(self, dl_port: str, erase=True, factory=False, external_path='', timeout=180):
        """
        进行QFile升级
        :param dl_port: 紧急下载口
        :param erase: 是否进行全擦
        ...
        package_name = self.cur_package_name if external_path == 'cur' else self.prev_package_name
        for p in os.listdir(os.path.join(os.getcwd(), 'firmware', external_path)):
            if os.path.isdir(os.path.join(os.getcwd(), 'firmware', external_path, p)):
                if factory:
                    if p.lower().endswith('factory') and package_name in p:
                        factory_package_path = os.path.join(os.getcwd(), 'firmware', external_path, p)
                else:
                    if not p.lower().endswith('factory') and package_name in p:
                        factory_package_path = os.path.join(os.getcwd(), 'firmware', external_path, p)
        if not factory_package_path:
            raise Exception("未找到{}升级包".format('工厂' if factory else '标准'))
        all_logger.info(f'当前升级包路径为{factory_package_path}')
    ...
    def qfil_upgrade_and_check(self, erase=False, factory=False, external_path='', flag=True):
        """
        进行QFIL升级并检查
        ...
        :param flag:  True：正常打开DM口转QDL口；False：代表模块已经进入紧急下载模式，此时无需打开DM口再转QDL口
        :return: True
        """
        ...
        # 切换紧急下载
        dl_port = self.switch_edl_mode(flag)
        ...
        while time.time() - start < 180:
            port_list_after_download = self.driver_check.get_port_list()
    ...
    def check_module_id(self):
        ...
        :return:
        """
        port_info = setupapi.ports()
        info = ''.join(re.findall(r'USB AT Port, (.*?)\)', "".join('%s' % name for name in port_info)))
        all_logger.info('查询硬件ID信息为{}'.format(info))
        if self.port_info != info:
            raise UpgradeOnOffError('查询AT口Pid等信息与配置不一致，查询返回{},配置为{}'.format(info, self.port_info))
    ...
    def windows_reset_module(self):
        """
        如果case升级失败会导致模块处于紧急下载模式，每次case执行前检查
        :return:
        """
        if self.get_download_port():
            ...
            if self.get_download_port():
                ...
                self.qfil(dl_port=self.get_download_port(), factory=True, external_path='cur')
                # 获取升级后的端口
                all_logger.info("对比升级前后端口")
                start = time.time()
                ports = {'at_port': self.at_port, 'dm_port': self.dm_port}
