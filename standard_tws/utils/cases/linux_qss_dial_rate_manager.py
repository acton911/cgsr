...
class LinuxQSSDialRateManager:
    def __init__(self, at_port, dm_port, wwan_path, gobinet_path, pcie_driver_path, qmi_usb_network_card_name,
                 mbim_usb_network_card_name, gobinet_usb_network_card_name, ecm_usb_network_card_name,
                 pcie_network_card_name,
                 local_network_card_name, qss_ip, local_ip, node_name, mme_file_name, enb_file_name, ims_file_name,
                 name_sub_version, chrome_driver_path):
        ...
        self.gobinet_path = gobinet_path
        ...
        self.mbim_usb_network_card_name = mbim_usb_network_card_name
        self.gobinet_usb_network_card_name = gobinet_usb_network_card_name
        self.ecm_usb_network_card_name = ecm_usb_network_card_name
        self.pcie_network_card_name = pcie_network_card_name
        ...
        self.speedtest_report_path = os.path.join(os.getcwd(), '5G_Speed_test_report.csv')
    ...
    def speedtest_qss(self):
        ...
        try:
            ...
            driver = webdriver.Chrome(chrome_options=chrome_options, executable_path=self.chrome_driver_path)  # 如果没有把chromedriver加入到PATH中，就需要指明路径
    ...
    def speedtest_report(self):
        """
        生成测速报告
        """
        all_logger.info(self.speedtest_data)
        df = pd.DataFrame(data=self.speedtest_records)
        df.to_csv(self.speedtest_report_path, index=False)  # noqa
        all_logger.info(self.speedtest_report_path)
    ...
    def get_speed_test_data_all(self, dial='QMI', network='NSA', interface='USB', ):
        ...
        # 'Band',
        self.speedtest_data['Band'] = self.enb_file_name.split('.')[0]
        # 'PCI',
        """
        AT+QENG="servingcell"
    ...
    def bound_network(self, network_type):
        ...
        if network_type in ["LTE", "WCDMA"]:  # 固定LTE或者WCDMA
            ...
            raise QSSDialRateError("不支持的网络类型设置")

        # 判断AT+QNWPREFCFG="nr5g_disable_mode"指令是否支持
        if 'ERROR' in at_data:
            raise QSSDialRateError('AT+QNWPREFCFG="nr5g_disable_mode"指令可能不支持')
    ...
    def open_qss_cmcc(self):
        ...
        if not start_result:
            raise QSSDialRateError('开启qss异常')
        ...
        if '3' != rerutn_bands['freq_band_ind'] or '41' != rerutn_bands['band']:
            raise QSSDialRateError("注网异常(B3n41)!")
        time.sleep(5)
    ...
    def open_qss_cu(self):
        ...
        if '3' != rerutn_bands['freq_band_ind'] or '78' != rerutn_bands['band']:
            raise QSSDialRateError("注网异常(B3n78)!")
        time.sleep(5)
        return start_time, qss_connection
    ...
    def check_wwan_driver(self, is_disappear=False):
        ...
        while True:
            ...
            if time.time() - check_time > 2:
                ...
                check_cmd.terminate()
                self.test_pass_flag = False
                self.test_fail_dial.append('QMI')
                return False
    ...
    def check_linux_qmi_and_driver_name(self, test_mode="USB"):
        ...
        network_card_name_now = self.qmi_usb_network_card_name if test_mode == 'USB' else self.pcie_network_card_name
    ...
    def qmi_quectel_cm(self, network_card_name, dial, test_mode):
        ...
        try:
            ...
            while time.time() - dial_connect_start_timestamp <= 120:
                ...
                self.test_pass_flag = False
                self.test_fail_dial.append(dial)
                return False
    ...
    def check_pcie_pci(self):
        ...
        if pcie_pci_x55 in pci_value or pcie_pci_x6x in pci_value:
            ...
            self.test_pass_flag = False
            self.test_fail_dial.append('PCIE')
            return False
    ...
    def insmod_pcie_qmi(self):
        ...
        if not check_result:
            ...
            if 'Makefile' not in return_value:
                ...
                self.test_pass_flag = False
                self.test_fail_dial.append("PCIE_QMI")
                return False
    ...
    def linux_mbim_start_init(self):  # Linux Mbim拨号前的初始化检查与设置
        time.sleep(60)  # 防止发AT口还未加载、发AT不生效等问题发生（AT配置已USB方式开机很慢）
        self.check_and_set_pcie_data_interface('usb')
    ...
    def set_linux_mbim_and_remove_driver(self):
        ...
        """
        network_types = ['qmi_wwan', 'qmi_wwan_q', 'cdc_ether']
        for name in network_types:
            all_logger.info(f"删除{name}网卡")
            subprocess.run(f"modprobe -r {name}", shell=True)
        """

    def check_linux_mbim_and_driver_name(self, test_mode="USB"):
        """
        检查是否是MBIM拨号方式，检查mbim驱动是否加载，检查WWAN驱动名称
        :return: None
        """
        if test_mode == 'USB':  # 兼容pcie
            ...
            for _ in range(timeout):
                ...
                self.test_pass_flag = False
                self.test_fail_dial.append('MBIM')
                return False
        all_logger.info("检查网卡名称")
        network_card_name_now = self.mbim_usb_network_card_name if test_mode == 'USB' else self.pcie_network_card_name
        s = subprocess.run("ip a | grep -o {}".format(network_card_name_now), shell=True, capture_output=True,
                           text=True)
        all_logger.info(s)
        if network_card_name_now not in s.stdout:
    ...
    def mbim_quectel_cm(self, network_card_name, dial, test_mode):
        self.at_handle.check_network()
        network = self.get_network()
        qcm = None
        exc_type = None
        ...
        try:
            ...
            while time.time() - dial_connect_start_timestamp <= 120:
                ...
                if connect_result:
                    # 记录拨号连接时间
                    all_logger.info("MBIM quectel-CM拨号成功！")
                    break
            else:
                # 断开连接失败次数+1
                ifconfig_value = subprocess.getoutput('ifconfig')
                all_logger.info('ifconfig\r\n{}'.format(ifconfig_value))
                all_logger.info("120s内MBIM quectel-CM拨号失败！")
                self.test_pass_flag = False
                self.test_fail_dial.append(dial)
                return False
            time.sleep(30)  # 等待拨号稳定
            ...
            if test_mode == "USB":
                ...
                self.check_linux_mbim_and_driver_name('pcie')
            # ping测试，兼容长拨和短拨
            for j in range(10):
                result = self.ping_get_connect_status(network_card_name=network_card_name)
                if not result and str(result) != '0.0':
                    self.udhcpc_get_ip(network_card_name)
                    # ping失败的话，失败次数+1
                    all_logger.info('ping失败，丢包率为：{}'.format(result))
                else:
                    # 记录丢包率
            ...
            qcm.terminate()
            # 恢复默认网卡
            ifconfig_up_value = subprocess.getoutput('ifconfig {} up'.format(self.local_network_card_name))  # 启用本地网卡
    ...
    def reset_usbnet(self):
        ...
        """
        all_logger.info("设置USBNET为0")
        self.at_handle.send_at('AT+QCFG="USBNET",0', 0.3)
        self.vbat()
        self.at_handle.check_urc()

    def linux_at_pcie_mbim_start_init(self):  # Linux Mbim拨号前的初始化检查与设置
        time.sleep(60)  # 防止发AT口还未加载、发AT不生效等问题发生
        self.driver_check.check_usb_driver()
        self.at_handle.send_at("ATE1", 3)
        self.check_and_set_pcie_data_interface('pcie')
    ...
    def insmod_pcie_mbim(self):
        ...
        """
        # 先检查，如果没有加载正确的驱动，则进行安装
        check_result = self.check_pcie_driver_mbim()
        if not check_result:
            return_value = subprocess.getoutput(f'ls {self.pcie_driver_path}')
            all_logger.info(f'ls {self.pcie_driver_path}')
            all_logger.info(f'\r\n{return_value}')
            if 'Makefile' not in return_value:
                all_logger.info('error pcie driver path, please check again!')
                self.test_pass_flag = False
                self.test_fail_dial.append("PCIE_MBIM")
                return False
    ...
    def gobinet_start_init(self):  # Linux gobinet拨号前的初始化检查与设置
        all_logger.info('检查GobiNet网卡驱动加载')
        self.check_and_set_pcie_data_interface('usb')
        result = self.check_linux_gobinet_and_driver_name()
        if not result:
            all_logger.info('设置GobiNet网卡驱动加载')
            self.set_linux_gobinet_and_remove_driver()
            self.install_gobinet_driver()
            self.vbat()
            self.at_handle.check_urc()
            self.load_gobinet_drive()
            all_logger.info('设置后再次检查GobiNet网卡驱动加载')
            self.check_linux_gobinet_and_driver_name()
    ...
    def install_gobinet_driver(self):
        """
        编译Gobinet驱动
        :return: None
        """
        # chmod 777
        all_logger.info(' '.join(['chmod', '777', self.gobinet_path]))
        s = subprocess.run(' '.join(['chmod', '777', self.gobinet_path]), shell=True, capture_output=True, text=True)
        all_logger.info(s.stdout)

        # make clean
        all_logger.info(' '.join(['make', 'clean', '--directory', self.gobinet_path]))
        s = subprocess.run(' '.join(['make', 'clean', '--directory', self.gobinet_path]), shell=True,
                           capture_output=True,
                           text=True)
        all_logger.info(s.stdout)

        # make install
        all_logger.info(' '.join(['make', 'install', '--directory', self.gobinet_path]))
        s = subprocess.run(' '.join(['make', 'install', '--directory', self.gobinet_path]), shell=True,
                           capture_output=True, text=True)
        all_logger.info(s.stdout)

    def set_linux_gobinet_and_remove_driver(self):
        """
        设置Gobinet拨号方式并且删除所有的网卡
        :return: None
        """
        time.sleep(5)  # 防止刚开机AT不生效
        all_logger.info("设置USBNET为0")
    ...
    def check_linux_gobinet_and_driver_name(self):
        """
        检查是否是gobinet拨号方式，检查gobinet驱动是否加载，检查gobinet驱动名称
        :return: None
        """
        all_logger.info("检查USBNET为0")
        for i in range(0, 5):
            ...
            if ',0' in usbnet:
                ...
                if i == 4:
                    all_logger.info('当前查询usbnet值不是GobiNet')
                    return False
                all_logger.info(f'当前查询指令返回{usbnet},等待5秒后再次查询')
        ...
        for _ in range(timeout):
            s = subprocess.run('lsusb -t', shell=True, capture_output=True, text=True)
            all_logger.info(s)
            if 'GobiNet' in s.stdout:
                all_logger.info("GobiNet驱动加载成功！")
                break
            time.sleep(1)
        else:
            all_logger.info(f"GobiNet驱动开机后{timeout}S未加载成功")
            return False

    def gobinet_quectel_cm(self, network_card_name, dial, test_mode):
        self.at_handle.check_network()
        network = self.get_network()
        qcm = None
        exc_type = None
        ...
        try:
            ifconfig_down_value = subprocess.getoutput(
                'ifconfig {} down'.format(self.local_network_card_name))  # 禁用本地网卡
            all_logger.info('禁用本地网卡ifconfig {} down\r\n{}'.format(self.local_network_card_name, ifconfig_down_value))
            time.sleep(20)
            # set quectel-CM
            qcm = QuectelCMThread(cmd='quectel-CM')
            qcm.setDaemon(True)
            ...
            while time.time() - dial_connect_start_timestamp <= 120:
                connect_result = self.get_ip_address(network_card_name="{}".format(network_card_name), ipv6_flag=False)
                if connect_result:
                    all_logger.info("Gobinet quectel-CM拨号成功！")
                    break
            else:
                ifconfig_value = subprocess.getoutput('ifconfig')
                all_logger.info('ifconfig\r\n{}'.format(ifconfig_value))
                all_logger.error("120s内Gobinet quectel-CM拨号失败！")
                self.test_pass_flag = False
                self.test_fail_dial.append(dial)
                return False
            time.sleep(30)  # 等待拨号稳定
            ...
            for j in range(10):
                ...
                all_logger.error("尝试10次重新获取IP并ping失败！")
                self.test_pass_flag = False
                self.test_fail_dial.append(dial)
                return False
            self.speedtest_dial(dial=dial, network=network, interface=test_mode)
    ...
    def ecm_start_init(self):  # Linux ECM拨号前的初始化检查与设置
        all_logger.info('检查ECM网卡驱动加载')
        self.check_and_set_pcie_data_interface('usb')
        result = self.check_linux_ecm_and_driver_name()
        if not result:
            all_logger.info('设置ECM网卡驱动加载')
            self.set_linux_ecm_and_remove_driver()
            self.vbat()
            self.at_handle.check_urc()
            all_logger.info('设置后再次检查ECM网卡驱动加载')
            self.check_linux_ecm_and_driver_name()

    def check_linux_ecm_and_driver_name(self):
        """
        检查是否是ECM拨号方式，检查ECM驱动是否加载，检查ECM驱动名称
        :return: None
        """
        all_logger.info("检查USBNET")
        for i in range(0, 5):
            time.sleep(5)
            usbnet = self.at_handle.send_at('AT+QCFG="USBNET"')
            if ',1' not in usbnet:
                if i == 4:
                    all_logger.info('当前查询usbnet值不是ECM')
                    return False
                all_logger.info(f'usbnet值检测{i + 1}次错误')
            else:
                all_logger.info('当前设置usbnet模式为1')
                break

        all_logger.info("检查ECM驱动加载")
        timeout = 10
        for _ in range(timeout):
            s = subprocess.run('lsusb -t', shell=True, capture_output=True, text=True)
            all_logger.info(s)
            if 'cdc_ether' in s.stdout:
                all_logger.info("ECM驱动加载成功")
                break
            time.sleep(1)
        else:
            all_logger.info("ECM驱动开机后30S未加载成功")
            all_logger.error(f'ECM驱动开机后{timeout}S加载失败')
            self.test_pass_flag = False
            self.test_fail_dial.append('ECM')
            return False

    def set_linux_ecm_and_remove_driver(self):
        """
        设置ECM拨号方式并且删除所有的网卡
        :return: None
        """
        time.sleep(5)  # 防止刚开机AT不生效
        all_logger.info("设置USBNET为1")
    ...
    def ecm_quectel_cm(self, network_card_name, dial, test_mode):
        self.at_handle.check_network()
        network = self.get_network()
        exc_type = None
        exc_value = None
        try:
            # 启用ECM拨号
            ifconfig_up_value1 = subprocess.getoutput('ifconfig {} up'.format(network_card_name))
            all_logger.info('启用ECM网卡ifconfig {} up\r\n{}'.format(network_card_name, ifconfig_up_value1))
            ifconfig_down_value = subprocess.getoutput(
                'ifconfig {} down'.format(self.local_network_card_name))  # 禁用本地网卡
            all_logger.info('禁用本地网卡ifconfig {} down\r\n{}'.format(self.local_network_card_name, ifconfig_down_value))
            time.sleep(20)
            ...
            while time.time() - dial_connect_start_timestamp <= 120:
                ...
                if connect_result:
                    # 记录拨号连接时间
                    all_logger.info("ECM quectel-CM拨号成功！")
                    break
            else:
                # 断开连接失败次数+1
                ifconfig_value = subprocess.getoutput('ifconfig')
                all_logger.info('ifconfig\r\n{}'.format(ifconfig_value))
                all_logger.error("120s内ECM quectel-CM拨号失败！")
                self.test_pass_flag = False
                self.test_fail_dial.append(dial)
                return False
            time.sleep(30)  # 等待拨号稳定
            ...
            # 禁用ECM拨号
            ifconfig_down_value = subprocess.getoutput('ifconfig {} down'.format(network_card_name))
            all_logger.info('禁用ECM网卡ifconfig {} down\r\n{}'.format(network_card_name, ifconfig_down_value))
            # 记录时间
            dial_disconnect_start_timestamp = time.time()
            while time.time() - dial_disconnect_start_timestamp <= 60:
                disconnect_result = self.get_ip_address(network_card_name="{}".format(network_card_name),
                                                        ipv6_flag=False, false_mode=True)
