...
class WindowsMBIMManager:
    ...
    def __init__(self, mbim_driver_name, ipv6_address, phone_number, params_path, at_port=None, dm_port=None):
        self.is_win11 = True if sys.getwindowsversion()[2] >= 22000 else False  # pylint: disable=E1101
        if self.is_win11:
        ...
        self.at_handler.switch_to_mbim(mbim_driver_name)
        self.modify_port()  # 有时候切换拨号模式会导致端口变化
        self.reset_cfun1()
    ...
    def enter_S3_S4_sleep(self, c=1, d=30, p=30, s=4):
        ...
        if 'Complete' not in return_value:
            raise MBIMError("PC进入{}模式失败！".format(s))
        time.sleep(3)
    ...
    def enter_modern_standby(self, c=1, d=30, p=30):
        ...
        if 'Connected Standby' not in return_value:
            raise MBIMError("PC进入modern standby失败！")
        time.sleep(3)

    @staticmethod
    def get_pwrtest_path():
        """
        ...
        for path, _, files in os.walk(check_path):  # pwrtest.exe
            for file in files:
                if file.endswith('.exe') and file.startswith("pwrtest"):
                    ...
                    return pwrtest_path
        raise MBIMError(f"在{check_path}中未找到pwrtest.exe，请检查工具文件夹是否存在")
    ...
    def get_and_compare_module_data(self):
        ...
        # 查询当前网卡
        self.at_handler.send_at('AT+QCFG="USBNET"')
        # 获取version
        version_data = self.at_handler.send_at("ATI")
        model = self.get_gmi()
        revision_data = ''.join(re.findall(r'Revision: (.*?)\s', version_data))
        # 获取imei
        imei_data = self.at_handler.send_at("AT+EGMR=0,7")
        imei = ''.join(re.findall(r'\+EGMR: "(\d+)"', imei_data))
        # 获取imsi
        cimi_data = self.at_handler.send_at("AT+CIMI")
        imsi = ''.join(re.findall(r'AT\+CIMI\s+(\d+)\s+OK\s+', cimi_data))
        # ICCID
        iccid_data = self.at_handler.send_at("AT+QCCID")
        iccid = ''.join(re.findall(r"\+QCCID:\s(\d+\S+)", iccid_data))
        # 本机号码
        cnum_data = self.at_handler.send_at("AT+CNUM")
        number = ''.join(re.findall(r'CNUM: ,"(.*?)"', cnum_data))
        # 参数拼接
        module_data = {
            "model": model,
            "driver_name": self.mbim_driver_name,
        ...
        """
        制造商:	Quectel
        型号:	Quectel RM520NGLAP
        固件:	RM520NGLAPR01A04M4G
        网络类型:	3GPP
        数据类:	UMTS, HSDPA, HSUPA, LTE, 5G,
        IMEI:	869710030002905
        移动电话号码:	+8618656504506
        IMSI:	460016505656068
        SIM 卡 ICCID:	89860120801319212830

        {
            'model': 'RM520N-GL',
            'version': 'RM520NGLAPR01A04M4G',
            'imei': '869710030002905',
            'imsi': '460016505656068',
            'iccid': '89860120801319212830',
            'phone_number': '+8618656504506'
        }

        E:\\Auto\\mbim_windows\\standard_tws>netsh mbn show readyinfo interface="手机网络"

        接口 手机网络 的就绪信息:
        -------------------------------------
            状态            : 准备就绪，可以接通电源并注册
            紧急模式   : 关闭
            订户 ID    : 460016505656068
            SIM ICC ID       : 89860120801319212830
            电话号码的位数: 1
                电话 #1             : +8618656504506
        ...
        data2 = os.popen('netsh mbn show readyinfo interface="{}"'.format(interface_name)).read()
        clipboard_data = data1 + data2
    ...
    def mbim_connect():
        ...
        for i in range(10):
            ...
            if '已连接' in data4 and '失败' not in data3:
                break
            all_logger.info(f"{data3}\r\n{data4}")
        else:
    ...
    def send_msg(self, content='123456'):
        ...
        :return:
        """
        self.at_handler.send_at('AT+QNWPREFCFG="NR5G_DISABLE_MODE",1', 10)  # 关闭SA网络
    ...
    def check_mbn(self):
        all_logger.info("检查MBN")
        while True:
            data = self.at_handler.send_at('at+qmbncfg="List"', timeout=15)
            if "ERROR" in data:
                continue
            else:
                all_logger.info(data)
                break
        data = self.at_handler.send_at('at+cgdcont?', timeout=15)
        all_logger.info(data)
