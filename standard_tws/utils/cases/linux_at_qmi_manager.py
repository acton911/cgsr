...
from utils.exception.exceptions import LinuxATQMIError
...
class LinuxATQMIManager:
    def __init__(self, quectel_cm_path, sim_info, version_name, network_card_name, local_network_card_name,
        ...
        self.PCIE = Reboot_PCIE(pcie_driver_path, '/dev/ttyUSBAT', '/dev/ttyUSBDM',
                                r'/sys/bus/pci/devices/0000\:01\:00.0')
    ...
    def check_pcie_driver(self):
        ...
        for i in driver_list:
            if i in driver_value:
                ...
                raise LinuxATQMIError(f'PCIE驱动检测失败，未检测到{i}驱动')
        else:
    ...
    def check_pcie_pci():
        ...
        if pcie_pci_x55 in pci_value or pcie_pci_x6x in pci_value:
            ...
            raise LinuxATQMIError('PCI检测失败，未检测到相关信息')
    ...
    def check_pcie_port(self, device_id='030'):
        ...
        if '5.0 GT/s PCIe' in pci_port_value:
            ...
            if '5.0 GT/s PCIe' in pci_port_value:
                ...
                raise LinuxATQMIError('PCI接口速率，查询失败')
    ...
    def check_pcie_data_interface(self):
        ...
        if '"data_interface",1' in data_interface_value:
            ...
            raise LinuxATQMIError(f'data_interface信息查询值不一致,查询信息为{data_interface_value}')
    ...
    def check_module_info(self):
        ...
        if re_qccid in sys_qccid:
            ...
            raise LinuxATQMIError(f'QCCID信息查询值与系统不一致,查询信息为{re_qccid},系统信息为{sys_qccid}')
        ...
        if release_name == sys_release_name:
            ...
            raise LinuxATQMIError(f'查询版本信息与系统上信息不一致,查询信息为{release_name},系统信息为{sys_release_name}')
        ...
        for i in range(30):
            ...
            if is_success:
                if ip:
                    ...
                    if i == 29:
                        raise LinuxATQMIError(f'获取IP地址异常,未获取到IP地址,ifconfig -a返回{ifconfig_value}')
            else:
                if not ip:
                    ...
                    if i == 29:
                        raise LinuxATQMIError(f'异常,当前未进行拨号,但获取到IP地址{ip}')
            time.sleep(2)
    ...
    def check_network_pcie(self):
        ...
        while True:
            ...
            if cops_value != '':
                ...
                at_logger.info(f"当前小区信息：{cell_value}")
                time.sleep(1)
                return cops_value
    ...
    def check_netcard(self):
        ...
        :return: True
        """
        all_logger.info(self.network_card_name)
        ifconfig_value = os.popen('ifconfig -a').read()
        if self.network_card_name in ifconfig_value:
            ...
            raise LinuxATQMIError(f'执行ifconfig -a未查询到网卡:{self.network_card_name}')
    ...
    def check_intranet(self, intranet_ip='192.168.11.252'):
        ...
        if not intranet_flag:
            raise LinuxATQMIError('The intranet connect FAIL, please check the intranet!')
    ...
    def bound_network(self, network_type, timeout=180):
        ...
        if network_type in ["LTE", "WCDMA"]:  # 固定LTE或者WCDMA
            ...
            raise LinuxATQMIError("不支持的网络类型设置")

        # 判断AT+QNWPREFCFG="nr5g_disable_mode"指令是否支持
        if 'ERROR' in at_data:
            raise LinuxATQMIError('AT+QNWPREFCFG="nr5g_disable_mode"指令可能不支持')
        ...
        while time.time() - start_timestamp < timeout:
            ...
            raise LinuxATQMIError(f"{timeout}秒内注册{network_type}网络类型失败，请检查当前网络环境是否支持和SIM卡是否支持")
        ...
        with serial.Serial(self.at_port, baudrate=115200, timeout=0) as __at_port:
            ...
            while True:
                ...
                if time.time() - start_time > timout:
                    raise LinuxATQMIError(f'{timout}S内未检测到{keyword1},{keyword2}')
