...
from utils.exception.exceptions import LinuxPcieQMIError
...
class LinuxPcieQMIManager:
    ...
    def check_pcie_driver():
        ...
        if '0306' not in lspci_value:
            raise LinuxPcieQMIError('执行lspci未识别到0306')
        ...
        for i in driver_list:
            if i in driver_value:
                ...
                raise LinuxPcieQMIError('PCIE驱动检测失败，未检测到{}驱动'.format(i))
        else:
    ...
    def insmod_pcie_qmi(self):
        ...
        if 'Makefile' not in return_value:
            raise LinuxPcieQMIError('error pcie driver path, please check again!')
        ...
        pci_value = subprocess.getoutput('insmod {}/pcie_mhi.ko'.format(self.pcie_driver_path))
        all_logger.info('insmod {}/pcie_mhi.ko\r\n{}'.format(self.pcie_driver_path, pci_value))
    ...
    def check_module_info(self):
        ...
        if re_qccid in sys_qccid:
            ...
            raise LinuxPcieQMIError('QCCID信息查询值与系统不一致,查询信息为{},系统信息为{}'.format(re_qccid, sys_qccid))
        ...
        if release_name == sys_release_name:
            ...
            raise LinuxPcieQMIError('查询版本信息与系统上信息不一致,查询信息为{},系统信息为{}'.format(release_name, sys_release_name))

    # def dial(self, times=10, is_success=True, cmd='quectel-CM', ipv6_flag=False):
    #     """
    #     拨号后进行Ping业务
    #     times: ping的时长
    ...
    #         if not ipv6_flag:
    #             self.local_net_down()
    #         q = QuectelCMThread(cmd)
    #         q.setDaemon(True)
    #         q.start()
    #         time.sleep(20)
    #         if is_success:
    #             LinuxAPI().ping_get_connect_status(ipv6_flag, network_card_name=self.network_card_name, times=times)
    #         else:
    #             self.get_ip_address()
    ...
    def dial(self, times=10, is_success=True):
        ...
        try:
            ...
            self.local_net_up()
            time.sleep(10)
            all_logger.info('udhcpc -i {}\r\n'.format(self.local_network_card_name))
    ...
    def make_dial_tool(self):
        ...
        s = subprocess.run(' '.join(['make', 'clean', '--directory', self.quectel_cm_path]), shell=True,
                           capture_output=True, text=True)
        all_logger.info(s.stdout)

        # make install
        all_logger.info(' '.join(['make', 'install', '--directory', self.quectel_cm_path]))
        s = subprocess.run(' '.join(['make', '--directory', self.quectel_cm_path]), shell=True, capture_output=True,
                           text=True)
        all_logger.info(s.stdout)
        ...
        for i in range(30):
            ...
            if is_success:
                if ip:
                    ...
                    if i == 29:
                        raise LinuxPcieQMIError('获取IP地址异常,未获取到IP地址,ifconfig -a返回{}'.format(ifconfig_value))
            else:
                if not ip:
                    ...
                    if i == 29:
                        raise LinuxPcieQMIError('异常,当前未进行拨号,但获取到IP地址{}'.format(ip))
            time.sleep(2)
    ...
    def check_cfun(self, value):
        ...
        if str(value) in cfun_value:
            ...
            raise LinuxPcieQMIError('CFUN值异常')
    ...
    def check_intranet(self, intranet_ip='192.168.11.252'):
        ...
        if not intranet_flag:
            raise LinuxPcieQMIError('The intranet connect FAIL, please check the intranet!')
    ...
    def check_network_pcie(self):
        ...
        while True:
            ...
            cell_value = self.send_at_pcie('AT+QENG="servingcell"')
            ...
            if cops_value != '':
                ...
                time.sleep(1)
                self.send_at_pcie('AT+CREG?;+CGREG?;+CEREG?;+CSQ;+QNWINFO')
                return cops_value
    ...
    def check_netcard(self):
        """
        检查网卡驱动是否正常加载
        :return: True
        """
        ifconfig_value = os.popen('ifconfig -a').read()
        if self.network_card_name in ifconfig_value:
            all_logger.info('网卡加载正常')
            return True
        else:
            all_logger.info(ifconfig_value)
            raise LinuxPcieQMIError(f'执行ifconfig -a未查询到网卡:{self.network_card_name}')
    ...
    def local_net_up(self):
        ...
        for i in range(10):
            try:
                ...
                # ifconfig_value = os.popen('ifconfig').read()
            ...
            raise LinuxPcieQMIError('开启本地网失败')
    ...
    def local_net_down(self):
        ...
        for i in range(10):
            ...
            raise LinuxPcieQMIError('关闭本地网失败')
    ...
    def bound_network(self, network_type, timeout=180):
        ...
        if network_type in ["LTE", "WCDMA"]:  # 固定LTE或者WCDMA
            ...
            raise LinuxPcieQMIError("不支持的网络类型设置")

        # 判断AT+QNWPREFCFG="nr5g_disable_mode"指令是否支持
        if 'ERROR' in at_data:
            raise LinuxPcieQMIError('AT+QNWPREFCFG="nr5g_disable_mode"指令可能不支持')
        ...
        while time.time() - start_timestamp < timeout:
            ...
            raise LinuxPcieQMIError("{}秒内注册{}网络类型失败，请检查当前网络环境是否支持和SIM卡是否支持".format(timeout, network_type))
    ...
    def send_msg(self, content='123456'):
        ...
        self.readline_keyword('+CMTI', timout=300)
        time.sleep(5)
        ...
        with serial.Serial(self.at_port, baudrate=115200, timeout=0) as __at_port:
            ...
            while True:
                ...
                if time.time() - start_time > timout:
                    raise LinuxPcieQMIError('{}S内未检测到{},{}'.format(timout, keyword1, keyword2))
