...
class Windows11EsimManager:
    ...
    def __init__(self, at_port, dm_port, mbim_driver, profile_info):
        ...
        self.pageMobileBroadband = Win11PageMobileBroadband()
        self.at_handle.send_at('AT+QURCCFG="URCPORT","USBMODEM"')
        self.check_slot()
    ...
    def check_slot(self):
        ...
        if '+QUIMSLOT: 1' in slot_number:
            self.at_handle.send_at('AT+QUIMSLOT=2', timeout=30)
            try:
                self.at_handle.readline_keyword('PB DONE', timout=50)  # 如果未激活ESIM，则卡二无卡，无法获取URC上报
    ...
    def windows_change_slot(self):
        ...
        :return:
        """
        self.pageMobileBroadband.mobile_page_up()   # 电脑打开网络页面可能不显示卡槽信息，默认首先向上翻页
    ...
    def check_qccid(self, iccid, is_ready=True, profile_name=None):
        """
        检查qccid号
        :param iccid: 预期的iccid号
        :param is_ready: profile文件是否应用情况下检测
        :param profile_name: profile文件名，多文件存在时，需要先点击该文件展开再检查iccid号
        :return: 返回ICCID号
        """
        if profile_name:
            exec(f'self.pageMobileBroadband.{profile_name}()')
        else:
            send_keys('{ENTER}')
        qccid_value = ''
        if is_ready:
            for i in range(10):
                ...
                if re.findall(r'QCCID: (\d+)', qccid_value):
                    break
                time.sleep(3)
            try:
                qccid_num = re.findall(r'QCCID: (\d+)', qccid_value)[0]
            except IndexError as e:
                all_logger.info(e)
    ...
    def check_imsi(self, is_ready=True):
        ...
        if is_ready:
            imsi_number = ''
            for i in range(10):
                imsi_number = self.at_handle.send_at('AT+CIMI', 3)
            ...
            if 'CME ERROR' not in self.at_handle.send_at('AT+CIMI', 3):
    ...
    def reset_activate_profile(self):
        ...
        time.sleep(20)  # 等待一会再检查profile文件
    ...
    def enable_auto_connect_and_disconnect(self):
        """
        开启自动连接，并且点击断开连接
        """
        ...
        self.page_main.click_esim_enable_auto_connect()
    ...
    def click_enable_airplane_mode_and_check(self):
        ...
        if flag:
            all_logger.info("模块进入飞行模式检查失败")
            raise WindowsEsimError("模块进入飞行模式检查失败")
        else:
            all_logger.info('模块进入飞行模式检查正常')
            return True
    ...
    def click_disable_airplane_mode_and_check(self):
        ...
        if flag:
            all_logger.info('模块退出飞行模式失败')
            raise WindowsEsimError("模块退出飞行模式检查失败")
        else:
            all_logger.info('模块退出飞行模式正常')
            return True

    def exit_airplane_mode_confirm(self):
        """
        确保退出飞行模式
        """
        if self.page_main.element_airplane_mode_button.get_toggle_state() == 0:
            all_logger.info("当前已退出飞行模式，无需退出")
        else:
            all_logger.info("点击飞行模式按钮")
            self.page_main.element_airplane_mode_button.click_input()  # 点击飞行模式图标
            time.sleep(3)  # 等待mobile_network_button状态切换
    ...
    def add_profile(self, activate_code, iccid, profile_name):
        ...
        self.pageMobileBroadband.edit_esim_profile(profile_name, True)
        self.check_qccid(iccid, profile_name=profile_dict[profile_name])
    ...
    def cfun_1_1(self):
        ...
        self.driver_check.check_usb_driver_dis()
        self.driver_check.check_usb_driver()
        time.sleep(10)  # 笔电上报URC很快，可能检测不到，不检测URC，等待一会后进入下一步
...
class CheckUrc(Thread):
    ...
    def __init__(self, at_port, is_inistat=False):
        ...
        self.at_port = at_port
        self.at_handle = ATHandle(at_port)
        self.check_urc = ['+CPIN: READY', '+QIND: PB DONE']
        self.error_msg = ''
        ...
        try:
            ...
            while True:
                ...
                if time.time() - start_time > 20:
                    ...
                    all_logger.info(f'收到URC为{return_value_cache}')
