import time
from utils.exception.exceptions import GPIOError
...
class GPIO:
    ...
    def __init__(self):
        ...
        self.pin_rtl_8125 = self.pin['RTL_8125'] if os.name == 'posix' else None
        ...
        self.pin_led_r = self.pin['LED_R']
        self.pin_led_g = self.pin['LED_G']
        self.pin_led_b = self.pin['LED_B']
    ...
    def set_dtr_high_level(self):
        ...
        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置DTR引脚{self.pin_dtr}为高电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置DTR引脚{self.pin_dtr}为高电平失败，JSON: {r.json()}")
        else:
    ...
    def set_dtr_low_level(self):
        ...
        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置DTR引脚{self.pin_dtr}为低电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置DTR引脚{self.pin_dtr}为低电平失败，JSON: {r.json()}")
        else:
    ...
    def set_rtl_8125_high_level(self):
        all_logger.info('设置RTL8125引脚为高电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_rtl_8125, "level": 1}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置RTL_8125引脚{self.pin_rtl_8125}为高电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置RTL_8125引脚{self.pin_rtl_8125}为高电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置RTL_8125引脚为高电平成功，状态码: {r.status_code}，返回值:{r.json()}')

    def set_rtl_8125_low_level(self):
        all_logger.info('设置RTL8125引脚为低电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_rtl_8125, "level": 0}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置RTL_8125引脚{self.pin_rtl_8125}为低电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置RTL_8125引脚{self.pin_rtl_8125}为低电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置RTL_8125引脚为低电平成功，状态码: {r.status_code}，返回值:{r.json()}')
    ...
    def get_wwan_led_gpio_level(self):
        all_logger.info('获取WWAN LED引脚电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_wwan_led}
        r = requests.post('http://127.0.0.1:55555/get', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"获取NET STATUS引脚{self.pin_wwan_led}电平失败，status_code: {r.status_code}")

        wwan_led_status = r.json()
        all_logger.info(f"获取NET STATUS引脚{self.pin_wwan_led}电平状态：{wwan_led_status}")
        return wwan_led_status

    def set_led_r_high_level(self):
        all_logger.info('设置LED_R引脚为高电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_led_r, "level": 1}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置LED_R引脚{self.pin_led_r}为高电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置LED_R引脚{self.pin_led_r}为高电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置LED_R引脚为高电平成功，状态码: {r.status_code}，返回值:{r.json()}')

    def set_led_r_lower_level(self):
        all_logger.info('设置LED_R引脚为低电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_led_r, "level": 0}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置LED_R引脚{self.pin_led_r}为低电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置LED_R引脚{self.pin_led_r}为低电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置LED_R引脚为低电平成功，状态码: {r.status_code}，返回值:{r.json()}')

    def set_led_g_high_level(self):
        all_logger.info('设置LED_G引脚为高电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_led_g, "level": 1}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置LED_G引脚{self.pin_led_g}为高电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置LED_G引脚{self.pin_led_g}为高电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置LED_G引脚为高电平成功，状态码: {r.status_code}，返回值:{r.json()}')

    def set_led_g_lower_level(self):
        all_logger.info('设置LED_G引脚为低电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_led_g, "level": 0}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置LED_G引脚{self.pin_led_g}为低电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置LED_G引脚{self.pin_led_g}为低电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置LED_G引脚为低电平成功，状态码: {r.status_code}，返回值:{r.json()}')

    def set_led_b_high_level(self):
        all_logger.info('设置LED_B引脚为高电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_led_b, "level": 1}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置LED_B引脚{self.pin_led_b}为高电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置LED_B引脚{self.pin_led_b}为高电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置LED_B引脚为高电平成功，状态码: {r.status_code}，返回值:{r.json()}')

    def set_led_b_lower_level(self):
        all_logger.info('设置LED_B引脚为低电平')

        # 请求ESP32线程
        requests_data = {"id": self.pin_led_b, "level": 0}
        r = requests.post('http://127.0.0.1:55555/set', json=requests_data)

        # 状态码异常
        if r.status_code != 200:
            raise GPIOError(f"设置LED_B引脚{self.pin_led_b}为低电平失败，status_code: {r.status_code}")

        # 返回值异常，返回数据为 {'error': f"fail to set GPIO x level"}
        if r.json().get('error', False):
            raise GPIOError(f"设置LED_B引脚{self.pin_led_b}为低电平失败，JSON: {r.json()}")
        else:
            all_logger.info(f'设置LED_B引脚为低电平成功，状态码: {r.status_code}，返回值:{r.json()}')


if __name__ == '__main__':
    gpio = GPIO()
    gpio.set_led_r_lower_level()
    time.sleep(1)
    gpio.set_led_r_high_level()
    time.sleep(1)
    gpio.set_led_g_lower_level()
    time.sleep(1)
    gpio.set_led_g_high_level()
    time.sleep(1)
    gpio.set_led_b_lower_level()
    time.sleep(1)
    gpio.set_led_b_high_level()
