...
token = "17bddb93-5db1-498e-99ee-81ed6c04b94e"
platform = "SDX55"
snapshot_case = []
...
cases = dict()
r = requests.get("https://tws.quectel.com:8152/api/qt-case/casegroups/5g"
...
for seq, (case_id, case_desc) in enumerate(cases.items()):
    ...
    for d in ids:
        ...
        if r.json()['data']['type_exec'] == 'apy' or\
                r.json()['data']['type_exec'] == 'aa' or\
                r.json()['data']['type_exec'] == 'ado' or\
                r.json()['data']['type_exec'] == 'ape':
            auto_all += 1
            if r.json()['data']['level'] == 'p0' or r.json()['data']['level'] == 'p6':  # p6 为P0-ALLATC:
                auto_p0 += 1
            elif r.json()['data']['level'] == 'p1':
                auto_p1 += 1
            elif r.json()['data']['level'] == 'p2':
                auto_p2 += 1
            elif r.json()['data']['level'] == 'p3':
                auto_p3 += 1
            else:
                auto_others += 1
        else:
    ...
    df.loc[seq, 'all'] = manual_all + auto_all
    df.loc[seq, 'auto_all'] = auto_all
    df.loc[seq, 'auto_p0'] = auto_p0
    df.loc[seq, 'auto_p1'] = auto_p1
    df.loc[seq, 'auto_p2'] = auto_p2
    df.loc[seq, 'auto_p3'] = auto_p3
    df.loc[seq, 'auto_others'] = auto_others
    df.loc[seq, 'manual_all'] = manual_all
    df.loc[seq, 'manual_p0'] = manual_p0
    df.loc[seq, 'manual_p1'] = manual_p1
    df.loc[seq, 'manual_p2'] = manual_p2
    df.loc[seq, 'manual_p3'] = manual_p3
    df.loc[seq, 'manual_others'] = manual_others

    logger.info(' '.join([case_desc,
                          ...
                          '自动P2:' + str(auto_p2),
                          '自动P3:' + str(auto_p3),
                          '自动other:' + str(auto_others),
                          '手动:' + str(manual_all),
                          '手动P0:' + str(manual_p0),
                          '手动P1:' + str(manual_p1),
                          '手动P2:' + str(manual_p2),
                          '手动P3:' + str(manual_p3),
                          '手动other:' + str(manual_others)]))
