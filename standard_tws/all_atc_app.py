...
def all_atc_qfil_standard(**param):
    """
    升级标准版本
    :param param:
    :return:
...
def all_atc_qfirehose_erase(**param):
    """
    qfirehose全擦工厂
    :param param:
    :return:
...
def all_atc_qfirehose_standard(**param):
    """
    qfirehose标准
    :param param:
    :return:
    """
    qfirehose = QFirehose(at_port=param['at_port'], dm_port=param['dm_port'], firmware_path=param['firmware_path'],
                          factory=False, package_name=name_sub_version, ati=param['ati'], csub=param['csub'])
    qfirehose.linux_upgrade()


all_logger.info('进行测试项:{}: {}'.format(func, case_setting['summary']))
if func == 'all_atc_qfil_erase':
    if os.name == 'nt':
        all_atc_qfil_erase(**params_dict)
    else:
        all_atc_qfirehose_erase(**params_dict)
else:
    if os.name == 'nt':
        all_atc_qfil_standard(**params_dict)
    else:
        all_atc_qfirehose_standard(**params_dict)
at = ATHandle(at_port)
at.get_mbn_status()
