...
params_dict = {'at_port': at_port, 'dm_port': dm_port, 'firmware_path': firmware_path,
               'ati': ati, 'csub': csub, 'package_name': package_name}

laptop = is_laptop(args_5g.qgmr)  # 判断是否是笔电，是True；不是False


def backupRestore_qfil_erase(**param):
    """
    全擦先升级工厂包，后升级标准包
    """
    if laptop:
        ...
        qfirehose = QFirehose(at_port=param['at_port'], dm_port=param['dm_port'], firmware_path=param['firmware_path'],
                              factory=True, ati=param['ati'], csub=param['csub'], package_name=param['package_name'])
        qfirehose.linux_upgrade()
        qfirehose.factory_to_standard()
        qfirehose.linux_upgrade()


def backupRestore_qfil_standard(**param):
    """
    升级标准版本
    :param param:
    :return:
    ...
    if laptop:
        fw = FW(at_port=param['at_port'], dm_port=param['dm_port'], firmware_path=param['firmware_path'],
                factory=False, ati=param['ati'], csub=param['csub'])
        fw.upgrade()
    elif os.name == "nt":
        qfil = QFIL(at_port=param['at_port'], dm_port=param['dm_port'], firmware_path=param['firmware_path'],
                    factory=False, ati=param['ati'], csub=param['csub'])
        ...
        qfirehose = QFirehose(at_port=param['at_port'], dm_port=param['dm_port'], firmware_path=param['firmware_path'],
                              factory=False, ati=param['ati'], csub=param['csub'], package_name=param['package_name'])
        qfirehose.linux_upgrade()


all_logger.info('进行测试项:{}: {}'.format(func, case_setting['summary']))
if func == 'backupRestore_qfil_erase':
    backupRestore_qfil_erase(**params_dict)
else:
    backupRestore_qfil_standard(**params_dict)

# X6X要求设置dump模式
if not laptop:
    all_logger.info("BackupRestore 当前非笔电版本，设置DUMP模式")
    at = ATHandle(at_port)
    at.set_dump_0()
    at.get_mbn_status()
